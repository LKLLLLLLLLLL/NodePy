...
<script lang="ts" setup>
    ...
    import type { ShiftNodeData } from '@/types/nodeTypes'


    const props = defineProps<NodeProps<ShiftNodeData>>()
    ...
    const periods = ref(props.data.param.periods)
    const periodsDisabled = computed(() => hasInputEdge(props.id, 'periods'))
    ...
    const onUpdatePeriods = () => {
        props.data.param.periods = periods.value
    }
    ...
    watch(() => JSON.stringify(props.data.error), () => {
        ...
        handleParamError(props.data.error, errMsg, colHasErr, periodsHasErr, result_colHasErr)
...
<style lang="scss" scoped>
    ...
    .ShiftNodeLayout {
        ...
        .data {
            ...
            .col, .periods, .result_col {
                padding: 0 $node-padding-hor;
                margin-bottom: $node-margin;
            }
        }
    }
    .all-handle-color[data-handleid="periods"] {
