<template>
    <div class="ColWithBoolBinOpNodeLayout nodes-style" :class="[{'nodes-selected': selected}, {'nodes-dbclicked': data.dbclicked}]">
        <NodeTitle node-category="compute">列布尔二元运算节点</NodeTitle>
        ...
        <div class="data">
            ...
            <div class="input-bool port">
                <div class="input-port-description">
                    布尔输入B
                </div>
                <Handle id="bool" type="target" :position="Position.Left" :class="[`${bool_type}-handle-color`, {'node-errhandle': boolHasErr.value}]"/>
            </div>
            <div class="op">
...
<script lang="ts" setup>
    ...
    import type { ColWithBoolBinOpNodeData } from '@/types/nodeTypes'


    const props = defineProps<NodeProps<ColWithBoolBinOpNodeData>>()
    ...
    const bool_type = computed(() => getInputType(props.id, 'bool'))
    ...
    const onSelectChangeOP = (e: any) => {
        const selected_op = op[e] as 'AND'|'OR'|'XOR'|'NUM_SUB_COL'|'COL_SUB_NUM'
    ...
    watch(() => JSON.stringify(props.data.error), () => {
        ...
        handleValidationError(props.id, props.data.error, errMsg, inputTableHasErr, boolHasErr)
    }, {immediate: true})
...
<style lang="scss" scoped>
    ...
    .ColWithBoolBinOpNodeLayout {
        height: 100%;
        .data {
            padding-top: $node-padding-top;
            padding-bottom: $node-padding-bottom;
            .input-table, .input-bool {
    ...
    .all-handle-color[data-handleid="bool"] {
