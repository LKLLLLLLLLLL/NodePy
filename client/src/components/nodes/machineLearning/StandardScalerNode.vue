<template>
    <div class="StandardScalerNodeLayout nodes-style" :class="[{'nodes-selected': selected}, {'nodes-dbclicked': data.dbclicked}]">
        ...
        <div class="data">
            ...
            <div class="output-scaled_table port">
                <div class="output-port-description">
                    表格输出
                </div>
                <Handle id="scaled_table" type="source" :position="Position.Right" :class="[`${schema_type}-handle-color`, {'node-errhandle': scaled_tableHasErr}]"/>
            </div>
        </div>
...
<script lang="ts" setup>
    ...
    import type { StandardScalerNodeData } from '@/types/nodeTypes'


    const props = defineProps<NodeProps<StandardScalerNodeData>>()
    ...
    const schema_type = computed(():server__models__schema__Schema__Type|'default' => props.data.schema_out?.['scaled_table']?.type || 'default')
    const scaled_tableHasErr = computed(() => handleOutputError(props.id, 'scaled_table'))
    ...
    watch(() => JSON.stringify(props.data.error), () => {
        ...
        handleParamError(props.data.error, errMsg, feature_colsHasErr)
