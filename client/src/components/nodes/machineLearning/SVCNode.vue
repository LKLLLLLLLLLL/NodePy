<template>
    <div class="SVCNodeLayout nodes-style" :class="[{'nodes-selected': selected}, {'nodes-dbclicked': data.dbclicked}]">
        <NodeTitle node-category="machineLearning">支持向量分类节点</NodeTitle>
        ...
        <div class="data">
            ...
            <div class="kernel">
                <div class="param-description" :class="{'node-has-paramerr': kernelHasErr.value}">
                    核函数类型
                </div>
                <NodepySelectMany
                    :options="kernel"
                    :default-selected="defaultSelectedKernel"
                    @select-change="(e: any) => updateSimpleSelectMany(data.param, 'kernel', kernel, e)"
                    class="nodrag"
                />
            </div>
...
<script lang="ts" setup>
    ...
    import type { SVCNodeData } from '@/types/nodeTypes'


    const props = defineProps<NodeProps<SVCNodeData>>()
    ...
    const kernel = ["rbf", "linear", "poly", "sigmoid"]
    const defaultSelectedKernel = ref(kernel.indexOf(props.data.param.kernel))
    ...
    watch(() => JSON.stringify(props.data.error), () => {
        ...
        handleParamError(props.data.error, errMsg, feature_colsHasErr, target_colHasErr, kernelHasErr)
...
<style lang="scss" scoped>
    ...
    .SVCNodeLayout {
        ...
        .data {
            ...
            .feature_cols, .target_col, .kernel {
