    ...
    <div class="AdvancePlotNodeLayout nodes-style" :class="[{'nodes-selected': selected}, {'nodes-dbclicked': data.dbclicked}]">
        ...
        <div class="data">
            ...
            <div class="hue_col">
                ...
                @select-change="onUpdateHue_col"
                @clear-select="clearSelectHue_col"
                class="nodrag"
                />
            </div>
            <div class="plot_type">
...
<script lang="ts" setup>
    ...
    const hue_col_default_selected = computed(() => hue_col_hint.value.indexOf(hue_col.value))
    const title = ref(props.data.param.title || '')
    const plot_type_options = ["bar", "count", "scatter", "strip", "swarm", "box", "violin", "hist"]
    const plot_type_options_chinese = ["柱状图", "计数图", "散点图", "散点条形图", "蜂群图", "箱线图", "小提琴图", "直方图"]
    ...
    const onSelectChange = (e: any) => {
        const oldType = props.data.param.plot_type
        const newType = plot_type_options[e]
        if(oldType !== 'count' && oldType !== 'hist' && (newType === 'count' || newType === 'hist')) {
            props.data.param.y_col = ''
            y_col.value = props.data.param.y_col
        }   //  if from other types to count or hist, clear y_col to avoid unknown action
        props.data.param.plot_type = newType as 'bar'| 'count'| 'scatter' | 'strip' | 'swarm' | 'box' | 'violin' | 'hist'
    }
    ...
    const onUpdateHue_col = (e: any) => {
        props.data.param.hue_col = hue_col_hint.value[e]
    }
    const onUpdateTitle = () => {
    ...
    const clearSelectHue_col = (resolve: any) => {
        props.data.param.hue_col = ''
        hue_col.value = props.data.param.hue_col
        resolve()
    }
    ...
    watch(() => JSON.stringify(props.data.error), () => {
        ...
        handleParamError(props.data.error, errMsg, x_colHasErr, y_colHasErr, hue_colHasErr, plot_typeHasErr)
...
<style lang="scss" scoped>
    ...
    .AdvancePlotNodeLayout {
        ...
        .data {
            ...
            .x_col, .y_col, .hue_col, .plot_type, .title {
