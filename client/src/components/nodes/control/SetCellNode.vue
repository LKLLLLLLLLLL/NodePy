<template>
    <div class="SetCellNodeLayout nodes-style" :class="[{'nodes-selected': selected}, {'nodes-dbclicked': data.dbclicked}]">
        <NodeTitle node-category="control">单元格设置节点</NodeTitle>
...
<script lang="ts" setup>
    ...
    import type { SetCellNodeData } from '@/types/nodeTypes'


    const props = defineProps<NodeProps<SetCellNodeData>>()
    ...
    watch(() => JSON.stringify(props.data.error), () => {
        ...
        handleValidationError(props.id, props.data.error, errMsg, inputTableHasErr, valueHasErr, inputRowHasErr)
    }, {immediate: true})
