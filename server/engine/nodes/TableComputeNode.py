...
from .Utils import Visualization, validate_no_index_column_conflict, CmpCondition
...
class TableCmpNode(BaseNode):
    ...
    def validate_parameters(self) -> None:
        ...
        self._cond = CmpCondition(
            op=self.op, left=("table_input", self.column), right=("value_input", None)
        )
            ...
            InPort(
                ...
                table_columns={
                    self.column: {
                        ...
                        Schema.ColumnType.STR,
                        Schema.ColumnType.BOOL,
                    }
                },
    ...
    def _compute_output_schema(self, input_schema: dict[str, Schema]) -> Schema:
        ...
        if self.result_col in input_cols:
            raise NodeValidationError(
                f"result_col '{self.result_col}' already exists in input table."
            )
        return Schema(
    ...
    def process(self, input: dict[str, Data]) -> dict[str, Data]:
        ...
        self.vis = Visualization(
            type=Visualization.Type.TABLE, payload=result_rows, node_id=self.id
        )
