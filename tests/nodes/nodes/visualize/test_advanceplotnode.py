...
def test_advanceplotnode_requires_y_col(node_ctor):
    ...
    with pytest.raises(NodeParameterError):
        node_ctor("AdvancePlotNode", id="ap_no_y", x_col="x", plot_type="scatter")


def test_advanceplotnode_allows_no_y_for_count(node_ctor):
    # plot_type 'count' may allow y_col to be None
    node = node_ctor("AdvancePlotNode", id="ap_count", x_col="x", plot_type="count")
    # Should not raise in construction
    assert node is not None


def test_advanceplotnode_hint_respects_plot_type(node_ctor):
    node_cls = node_ctor("AdvancePlotNode", id="ap_hint", x_col="x", plot_type="count")
    ...
    hint = node_cls.get_hint("AdvancePlotNode", {"input": schema}, {"plot_type": "count"})
...
def test_advanceplotnode_process_various_types(node_ctor, test_file_root, monkeypatch):
    ...
    for t in types:
        ...
        if t in {"count"}:
            node = node_ctor("AdvancePlotNode", id=f"ap_{t}", x_col="x", plot_type=t)
            schema = schema_from_coltypes({"x": ColType.STR, "y": ColType.INT, "h": ColType.STR})
        else:
            node = node_ctor("AdvancePlotNode", id=f"ap_{t}", x_col="x", y_col="y", hue_col="h", plot_type=t)
            schema = schema_from_coltypes({"x": ColType.STR, "y": ColType.INT, "h": ColType.STR})
...
def test_advanceplotnode_construction_normals_and_errors(node_ctor):
    # normals
    n1 = node_ctor("AdvancePlotNode", id="ap_ok1", x_col="x", y_col="y", plot_type="scatter")
    n2 = node_ctor("AdvancePlotNode", id="ap_ok2", x_col="x", plot_type="count")
    assert n1 is not None and n2 is not None

    # errors: missing/empty x_col
    import pytest
    with pytest.raises(Exception):
        node_ctor("AdvancePlotNode", id="ap_err1", x_col="  ", y_col="y", plot_type="scatter")

    # errors: y_col None for non-count plot type
    with pytest.raises(Exception):
        node_ctor("AdvancePlotNode", id="ap_err2", x_col="x", plot_type="scatter")

    # errors: validate parameters type mismatch when type attribute altered
    node = node_ctor("AdvancePlotNode", id="ap_err3", x_col="x", y_col="y", plot_type="bar")
...
def test_advanceplotnode_hint_error_paths():
    ...
    try:
        ...
        viz_mod.AdvancePlotNode.hint = orig
...
def test_advanceplotnode_hint_additional_normal(node_ctor):
    ...
    node = node_ctor("AdvancePlotNode", id="ap_hint2", x_col="x", y_col="y", plot_type="scatter")
    schema = schema_from_coltypes({"a": ColType.STR, "b": ColType.INT})
    hint = node.get_hint("AdvancePlotNode", {"input": schema}, {"plot_type": "scatter"})
    assert "x_col_choices" in hint
...
def test_advanceplotnode_infer_and_execute_error_cases(node_ctor):
    import pytest
    node = node_ctor("AdvancePlotNode", id="ap_exec_err", x_col="x", y_col="y", plot_type="scatter")
